...
export const getMark = () => {
  /**
     ...
     * @param {string} fontColor - 水印文本颜色
     */
  const setWaterMark = (str: string, createTime: boolean, container?: HTMLElement, fontColor?: string) => {
    // 创建唯一标识符id
    ...
    // 设置填充绘色
    cans.fillStyle = fontColor ? fontColor : "rgba(200, 200, 200, 0.40)";
    // 设置在绘制文本时使用当前文本基线
  ...
  // 只允许调用一次该方法
  const waterMark = (str: string, createTime: boolean, container?: HTMLElement, fontColor?: string) => {
    let id = setWaterMark(str, createTime, container, fontColor);
    // 定时检查是否存在具有相同标识符的元素
    setInterval(() => {
      if (document.getElementById(id) === null) {
        id = setWaterMark(str, createTime, container, fontColor);
      }
    }, 500);

    // 监听窗口大小调整, 确保水印的适应性
    const handleResize = () => {
      setWaterMark(str, createTime, container, fontColor);
    };
